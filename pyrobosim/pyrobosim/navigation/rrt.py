...
import numpy as np
import warnings
from typing import Optional, Tuple, TYPE_CHECKING, Any, List
...
class RRTPlannerPolygon:
    ...
    def plan(self, start: Pose, goal: Pose) -> Path:
        ...
        while not goal_found:
            ...
            n_near = self.graph_start.nearest(q_sample)  # type: ignore
            n_new = self.extend(n_near, q_sample)  # type: ignore
            connected_node = self.world.is_connectable(
                n_near.pose,  # type: ignore
                n_new.pose,
                self.collision_check_step_dist,
                self.max_connection_dist,
            )
            if connected_node:
                ...
                self.graph_start.add_edge(n_near, n_new)  # type: ignore

            # If bidirectional,
            # also connect a new node to the parent of the goal graph.
            if self.bidirectional:
                n_near_goal = self.graph_goal.nearest(q_sample)  # type: ignore
                n_new_goal = self.extend(n_near_goal, q_sample)  # type: ignore
                connected_node_goal = self.world.is_connectable(
                    n_near_goal.pose,  # type: ignore
                    n_new_goal.pose,
                    self.collision_check_step_dist,
                    self.max_connection_dist,
                )
                if connected_node_goal:
                    ...
                    self.graph_goal.add_edge(n_near_goal, n_new_goal)  # type: ignore
            ...
            if self.bidirectional:
                if connected_node:
                    ...
                    goal_found, n_goal_start_tree = self.try_connect_until(
                        self.graph_start, n_new, n_goal_goal_tree  # type: ignore
                    )

                if connected_node_goal and not goal_found:
                    # If we added a node to the goal tree,
                    # try connect to the start tree.
                    n_goal_start_tree = self.graph_start.nearest(n_new_goal.pose)  # type: ignore
        ...
        path_poses.reverse()
        if self.bidirectional:
            n = n_goal_goal_tree  # type: ignore
            path_built = False
            while not path_built:
                if n.parent is None:
                    path_built = True
    ...
    def sample_configuration(self) -> Optional[Pose]:
        """
        ...
        if n_rewire is not None:
            ...
            for e in graph.edges:
                if e.nodeA == n_tgt or e.nodeB == n_tgt:
                    ...
                    break
            graph.add_edge(n_tgt, n_tgt.parent)  # type: ignore
            self.n_rewires += 1
    ...
    def try_connect_until(
        ...
        while True:
            ...
            if self.rrt_connect:
                ...
                n_new = self.extend(n_curr, n_tgt.pose)  # type: ignore
                if self.world.is_connectable(
                    n_curr.pose,
                    n_new.pose,
                    self.collision_check_step_dist,
...
class RRTPlanner(PathPlannerBase):
    ...
    def __init__(self, **planner_config: Any) -> None:
        """
        ...
        if planner_config.get("grid", None):
            ...
            self.impl = RRTPlannerPolygon(**planner_config)  # type: ignore
    ...
    def plan(self, start: Any, goal: Any) -> Path:
        ...
        self.graphs = self.impl.get_graphs()  # type: ignore
        return self.latest_path
