...
class PRMPlannerPolygon:
    ...
    def connect_neighbors(self, node: Node) -> None:
        ...
        for other in self.graph.nodes:
            if node == other:
                continue
            if self.world.is_connectable(  # type: ignore
                node.pose,
                other.pose,
    ...
    def plan(self, start: Union[Pose, Node], goal: Union[Pose, Node]) -> Path:
        ...
        if self.compress_path:
            compressed_poses = reduce_waypoints_polygon(
                self.world, self.latest_path.poses, self.collision_check_step_dist  # type: ignore
            )
    ...
    def sample_configuration(self) -> Optional[Pose]:
        ...
        """
        return self.world.sample_free_robot_pose_uniform()  # type:ignore
...
class PRMPlanner(PathPlannerBase):
    ...
    def __init__(self, **planner_config: Any) -> None:
        ...
        if planner_config.get("grid", None):
            ...
            self.impl = PRMPlannerPolygon(**planner_config)  # type: ignore

    def plan(self, start, goal) -> Path:
        """
        Plans a path from start to goal.
        ...
        self.planning_time = time.time() - start_time  # type: ignore
